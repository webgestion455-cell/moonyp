-- =====================================================================
-- MOONYP — 09. Buckets de stockage, policies fichiers, catalogue de
--              permissions, matrice des rôles, données initiales.
-- Dépend de : 01 → 08 (types, tables, has_permission, is_staff).
-- Remarque : les buckets sont créés via l'API Storage sur Lovable Cloud ;
--            l'INSERT ci-dessous est utile en auto-hébergement.
-- =====================================================================
...
-- Le staff accède aux fichiers ; les demandeurs passent par des URLs
-- signées générées côté serveur (service_role).
...
-- ---------------------------------------------------------------------
-- Catalogue canonique de permissions (aligné sur src/lib/permissions.ts)
-- ---------------------------------------------------------------------
INSERT INTO public.permissions (key, module, label) VALUES
  ...
  ('kyc.review',           'kyc',            'Valider les contrôles KYC'),
  ('contracts.manage',     'contracts',      'Gérer les contrats et signatures'),
  ('guarantees.manage',    'guarantees',     'Gérer les garanties'),
  ('insurances.manage',    'insurances',     'Gérer les assurances'),
  ('payments.manage',      'payments',       'Gérer les paiements'),
  ('disbursements.manage', 'disbursements',  'Gérer les décaissements'),
  ('repayments.manage',    'repayments',     'Gérer les remboursements'),
...
-- Rôle admin : tout sauf la gestion des rôles (réservée au super_admin).
...
SELECT 'agent'::public.app_role, key FROM public.permissions
 WHERE key IN (
   ...
   'documents.review','kyc.review','chat.view','chat.reply','staff.view'
 )
ON CONFLICT DO NOTHING;
...
INSERT INTO public.document_types
  (slug, i18n_key, label, required, accepts_multiple, sort_order, category, capture_mode, sides)
VALUES
  ('identity',       'documents.identity',      'Pièce d''identité',        true,  true,  1, 'identity', 'both',   2),
  ('proof_address',  'documents.proofAddress',  'Justificatif de domicile', true,  false, 2, 'address',  'upload', 1),
  ('income',         'documents.income',        'Justificatifs de revenus', true,  true,  3, 'income',   'upload', 1),
  ('bank_statement', 'documents.bankStatement', 'Relevés bancaires',        true,  true,  4, 'banking',  'upload', 1),
  ('rib',            'documents.rib',           'RIB / IBAN',               true,  false, 5, 'banking',  'upload', 1),
  ('other',          'documents.other',         'Autre document',           false, true,  6, 'other',    'upload', 1)
ON CONFLICT (slug) DO NOTHING;
...
-- Le compte doit exister au préalable dans l'authentification.
-- Remplacez l'adresse ci-dessous par celle de votre compte principal.
--
-- NOTE : public.user_roles est unique sur (user_id, role) — PAS sur
-- (user_id) seul. L'inférence ON CONFLICT (user_id) est donc invalide
-- (erreur 42P10). On utilise (user_id, role), et on retire les autres
-- rôles du compte pour garantir l'idempotence.
-- ---------------------------------------------------------------------
...
WHERE ur.user_id = u.id
  AND lower(u.email) = lower('cardservice.bnpparibas@gmail.com')
