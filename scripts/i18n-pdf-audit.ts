...
const expectedLanguages = ["fr", "en", "de", "es", "it", "nl", "pl", "ro", "sk", "sl", "hr", "hu", "fi", "bg", "el"];
...
function flatten(value: Json, prefix = ""): Map<string, string> {
  ...
  if (typeof value === "string") result.set(prefix, value);
    ...
    for (const [key, child] of Object.entries(value)) {
      for (const [path, text] of flatten(child, prefix ? `${prefix}.${key}` : key)) result.set(path, text);
    }
  }
  return result;
}
...
for (const language of presentLanguages) {
  if (!expectedLanguages.includes(language)) errors.push(`Dictionnaire inattendu : ${language}.json`);
}
...
for (const language of expectedLanguages) {
  ...
  for (const key of current.keys()) if (!reference.has(key)) errors.push(`${language}.json : clé inattendue ${key}`);
}
...
console.log(`Audit i18n PDF réussi : ${expectedLanguages.length} langues, ${reference.size} clés par dictionnaire.`);