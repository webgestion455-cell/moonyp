...
export async function generateGuaranteeDocument(guaranteeId: string, final = false) {
  ...
  const { bytes, hash, pages } = await buildGuaranteePdf({
    ...
    version,
    issuedAt,
    borrower: fullName(app),
    borrowerAddress: address(app),
    borrowerEmail: app.email ?? "",
    borrowerPhone: app.phone,
    borrowerCountry: app.country,
    kind: guarantee.kind,
    guarantorName: guarantee.guarantor_name,
    amount: Number(guarantee.amount ?? guarantee.fee_amount ?? 0),
    feeAmount: Number(guarantee.fee_amount ?? 0),
    feeDescription: guarantee.fee_description,
    currency: guarantee.currency ?? "EUR",
    status: guarantee.status,
    paymentStatus: guarantee.payment_status,
    clientChoice: guarantee.client_choice,
    scheduledPaymentDate: guarantee.scheduled_payment_date,
    paymentValidatedAt: guarantee.payment_validated_at,
    paymentInstructions: guarantee.payment_instructions,
    loanAmount: Number(app.amount ?? 0),
    signature: signature ? { ...signature, documentHash: artifact.document_hash ?? null } : null,
  });
  const path = await storeGuaranteePdf(
    guarantee.application_id,
    `guarantee-v${version}${final ? "-signed" : ""}.pdf`,
    bytes,
  );
  const patch = final
    ? {
        ...
        signed_at: signature?.signedAt ?? guarantee.payment_validated_at ?? issuedAt,
        signature_name: signature?.name ?? null,
        signature_method: signature?.provider ?? "administrative_validation",
        signature_reference: signature?.reference ?? null,
      }
...
export async function generateInsuranceDocument(insuranceId: string, final = false) {
  ...
  const { data: app } = await supabaseAdmin
    .from("loan_applications")
    .select("*")
    .eq("id", insurance.application_id)
    .maybeSingle();
  if (!app) throw new Error("application_not_found");
  ...
  const { bytes, hash, pages } = await buildInsurancePdf({
    ...
    borrowerEmail: app.email ?? "",
    borrowerPhone: app.phone,
    borrowerBirthDate: app.birth_date,
    borrowerCountry: app.country,
    provider: insurance.provider,
    policyNumber: insurance.policy_number,
    coverage: insurance.coverage,
    monthlyPremium: Number(insurance.monthly_premium ?? 0),
    feeAmount: Number(insurance.fee_amount ?? 0),
    currency: insurance.currency ?? "EUR",
    required: Boolean(insurance.required),
    startsOn: insurance.starts_on,
    dueDate: insurance.due_date,
    status: insurance.status,
    paymentStatus: insurance.payment_status,
    clientChoice: insurance.client_choice,
    scheduledPaymentDate: insurance.scheduled_payment_date,
    months: Number(app.duration_months ?? 0),
    signature: signature ? { ...signature, documentHash: artifact.document_hash ?? null } : null,
  });
  const path = await storeInsurancePdf(
    insurance.application_id,
    `insurance-v${version}${final ? "-signed" : ""}.pdf`,
    bytes,
  );
  const patch = final
    ? {
        signed_storage_path: path,
        signed_document_hash: hash,
        signed_at: signature?.signedAt ?? insurance.validated_at ?? issuedAt,
        signature_name: signature?.name ?? null,
        signature_method: signature?.provider ?? "administrative_validation",
        signature_reference: signature?.reference ?? null,
      }
...
export async function signedCoverageUrl(
  _kind: DocumentKind,
  path: string | null,
): Promise<string | null> {
  if (!path) return null;
