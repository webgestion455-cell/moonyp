...
export function fuzzyIncludes(haystackTokens: readonly string[], needle: string): boolean {
  ...
  return haystackTokens.some(
    (t) => Math.abs(t.length - n.length) <= tolerance && levenshtein(t, n) <= tolerance,
  );
}
...
for (const [m, names] of Object.entries(MONTHS))
  for (const n of names) MONTH_LOOKUP.set(normalizeText(n), Number(m));
...
export function extractDates(text: string): ExtractedDate[] {
  ...
  for (const m of text.matchAll(
    /\b([\p{L}]{3,14})\.?\s+(\d{1,2})(?:st|nd|rd|th)?,?\s+(\d{4})\b/gu,
  )) {
 ...
 */
export function documentReferenceDate(
  text: string,
  nowIso: string,
): { date: string | null; future: string[]; all: string[] } {
  ...
  const past = dates
    ...
    .sort();
  return {
    date: past.length ? past[past.length - 1]! : null,
    future,
    all: dates.map((d) => d.iso),
  };
}
...
export function matchAddress(text: string, declared: DeclaredAddress): AddressMatch {
  ...
  const streetTokens = streetTokensAll.filter(
    (t) => !STREET_STOPWORDS.has(t) && !/^\d+[a-z]?$/.test(t) && t.length >= 3,
  );
  ...
  const score =
    Math.round(
      ((postalOk ? 35 : 0) + (cityOk ? 25 : 0) + streetRatio * 30 + (numberOk ? 10 : 0)) * 100,
    ) / 100;
  return {
    postal_code: postalOk,
    city: cityOk,
    street: streetOk,
    number: numberOk,
    street_ratio: Math.round(streetRatio * 100) / 100,
    score,
...
export function closestAmount(
  amounts: readonly number[],
  expected: number,
): { amount: number; deviation: number } | null {
  ...
  for (const a of amounts) {
    ...
    if (!best || dev < best.deviation)
      best = { amount: a, deviation: Math.round(dev * 1000) / 1000 };
  }
  return best;
}
...
export function classifyDocument(text: string): NatureGuess {
  ...
  for (const [nature, words] of Object.entries(NATURE_KEYWORDS) as [
    Exclude<DocumentNature, "unknown">,
    string[],
  ][]) {
    let hits = 0;
...
export function textQuality(text: string, minWords = 25): TextQuality {
  ...
  const readableWords = words.filter(
    (w) =>
      /^[\p{L}\p{N}][\p{L}\p{N}.,'’-]{1,}$/u.test(w) &&
      (w.match(/[\p{L}\p{N}]/gu) ?? []).length >= 3,
  );
