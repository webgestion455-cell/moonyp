...
import { adminSendInsurance } from "@/lib/guarantees.functions";
...
export function AdminFinancePanel({ applicationId, onChanged }: { applicationId: string; onChanged?: () => void }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [p, d] = await Promise.all([
        listPayments({ data: { application_id: applicationId } }),
        getDisb({ data: { application_id: applicationId } }),
      ]);
      setPayments(p);
      setDisb(d);
      if (!payoutAmount) setPayoutAmount(String(d.application.amount ?? ""));
    } catch (error) {
  ...
  }, [applicationId, getDisb, listPayments]);
  ...
  async function run(action: () => Promise<unknown>, success: string) {
    ...
    try {
      ...
      toast.error(error instanceof Error ? t(error.message, { defaultValue: error.message }) : "Action impossible");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="flex items-center gap-2 text-sm font-semibold">
            <Banknote className="h-4 w-4" /> Encaissements
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-2">
          {payments.length === 0 ? (
            ...
            payments.map((p) => (
              <div key={p.id} className="flex flex-col gap-2 rounded-lg border border-border p-3 sm:flex-row sm:items-center">
                ...
                <span className={`rounded-full px-2 py-0.5 text-[11px] font-medium ${STATUS_TONE[p.status] ?? "bg-muted"}`}>
                ...
                <div className="flex shrink-0 gap-2">
                  <Button
                    ...
                    onClick={() =>
                      run(() => updatePayment({ data: { id: p.id, status: "received" } }), "Encaissement confirmé")
                    }
                  >
                  ...
                  <Button
                    ...
                    className="text-xs"
                    disabled={busy}
                    onClick={() => run(() => updatePayment({ data: { id: p.id, status: "failed" } }), "Paiement rejeté")}
                  >
                    Rejeter
                  </Button>
                </div>
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="flex items-center gap-2 text-sm font-semibold">
            <ShieldCheck className="h-4 w-4" /> Assurance — prime et frais
          </CardTitle>
        </CardHeader>
        <CardContent className="grid gap-3 sm:grid-cols-2">
          ...
          <div>
            <Label className="text-xs">Garanties couvertes</Label>
            <Input
              className="mt-1.5"
          ...
          <div>
            <Label className="text-xs">Prime mensuelle (€)</Label>
            <Input
              className="mt-1.5"
              type="number"
              min={0}
              ...
              onChange={(e) => setInsurance({ ...insurance, monthly_premium: e.target.value })}
            />
          </div>
          <div>
            <Label className="text-xs">Frais de mise en place (€)</Label>
            <Input
              className="mt-1.5"
              type="number"
              min={0}
              ...
              onChange={(e) => setInsurance({ ...insurance, fee_amount: e.target.value })}
            />
          </div>
          <div className="sm:col-span-2">
            <Label className="text-xs">Détail des frais (visible client)</Label>
            <Textarea
              className="mt-1.5"
              rows={2}
              maxLength={500}
              value={insurance.fee_description}
              onChange={(e) => setInsurance({ ...insurance, fee_description: e.target.value })}
            />
          </div>
          <div>
            <Label className="text-xs">Échéance de règlement</Label>
            <Input
              className="mt-1.5"
              type="date"
              value={insurance.due_date}
              onChange={(e) => setInsurance({ ...insurance, due_date: e.target.value })}
            />
          </div>
          <div className="flex items-end">
            <Button
              size="sm"
              disabled={busy || insurance.monthly_premium === ""}
              onClick={() =>
                run(
                  () =>
                    sendInsurance({
                      data: {
                        ...
                        coverage: insurance.coverage || undefined,
                        monthly_premium: Number(insurance.monthly_premium || 0),
                        fee_amount: Number(insurance.fee_amount || 0),
                        fee_description: insurance.fee_description || undefined,
                        due_date: insurance.due_date || undefined,
                        currency: "EUR",
                        required: true,
                      },
                    }),
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="flex items-center gap-2 text-sm font-semibold">
            <Banknote className="h-4 w-4" /> Décaissement
          </CardTitle>
        </CardHeader>
          ...
          {current ? (
            <div className="space-y-2 rounded-lg border border-border p-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <Badge variant={current.status === "disbursed" ? "default" : "secondary"}>{current.status}</Badge>
              </div>
              <p className="text-sm">
                {formatMoney(Number(current.amount), current.currency ?? "EUR", locale)}
                {current.processed_at ? ` · ${new Date(current.processed_at).toLocaleString(locale)}` : ""}
              </p>
              {current.status !== "disbursed" && (
                <Button
                  ...
                  onClick={() => {
                    if (!window.confirm("Confirmez-vous que les fonds ont réellement été envoyés ?")) return;
                    void run(
                      () =>
                        confirm({
                          data: {
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              ...
              <div className="sm:col-span-2">
                ...
                <Input className="mt-1.5" value={payoutNotes} onChange={(e) => setPayoutNotes(e.target.value)} />
              </div>
              <div>
                <Button
                  size="sm"
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="flex items-center gap-2 text-sm font-semibold">
            <CalendarClock className="h-4 w-4" /> Échéancier de remboursement
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="flex flex-wrap items-end gap-3">
            <div>
              ...
              <Input className="mt-1.5" type="date" value={firstDue} onChange={(e) => setFirstDue(e.target.value)} />
            </div>
            <Button
              size="sm"
              variant="outline"
              ...
              onClick={() =>
                run(
                  () => generate({ data: { application_id: applicationId, first_due_date: firstDue } }),
                  "Échéancier généré",
                )
              }
            >
          ...
          {schedule.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[720px] text-sm">
                ...
                <tbody>
                  {schedule.map((row) => {
                    const status = (row as { status?: string }).status ?? (row.paid ? "paid" : "upcoming");
                    return (
                      <tr key={row.id} className="border-b border-border/60">
                        ...
                        <td>
                          <span className={`rounded-full px-2 py-0.5 text-[11px] ${STATUS_TONE[status] ?? "bg-muted"}`}>
                            {t(`finance.repayment.status.${status}`, { defaultValue: status })}
                          </span>
                        </td>
                                ...
                                () =>
                                  record({
                                    data: {
                                      ...
                                      amount: Number(row.amount) - Number((row as { paid_amount?: number }).paid_amount ?? 0),
                                    },
                                  }),
                          ...
                          <Button
                            ...
                            onClick={() =>
                              run(
                                () =>
                                  setInstallment({
                                    data: { application_id: applicationId, installment_id: row.id, status: "late" },
                                  }),
