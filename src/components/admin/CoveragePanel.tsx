    ...
    <Card>
      ...
      <CardContent className="space-y-5">
          ...
          <div className="grid gap-5 lg:grid-cols-2">
            ...
            <section className="rounded-xl border border-border p-4">
              ...
              {guarantee ? (
                <div className="mt-3">
                  <Field
                    ...
                    value={humanLabel(GUARANTEE_KIND_LABEL, guarantee.kind)}
                  />
                  <Field
                    label="Statut de la garantie"
                    value={humanLabel(STATUS_LABEL, guarantee.status, "En attente")}
                  />
                  <Field
                    label="Frais de couverture"
                    value={money(guarantee.fee_amount ?? guarantee.amount, guarantee.currency)}
                  />
                  <Field
                    label="Choix du client"
                    value={humanLabel(CHOICE_LABEL, guarantee.client_choice, "En attente")}
                  />
                  {guarantee.document_hash && (
                    <Field
                      label="Empreinte SHA-256"
                      value={`${guarantee.document_hash.slice(0, 16)}…`}
                    />
                  )}
                  <Field
                    ...
                    value={
                      guarantee.scheduled_payment_date
                        ? new Date(guarantee.scheduled_payment_date).toLocaleDateString("fr-FR")
                        : "—"
                    }
                  />
                  ...
                  <Field
                    ...
                    value={
                      guarantee.payment_validated_at
                        ? new Date(guarantee.payment_validated_at).toLocaleString("fr-FR")
                        : "—"
                    }
                  />
                </div>
                ...
                <p className="mt-3 text-sm text-muted-foreground">
                  Aucune garantie envoyée. Saisissez les frais de couverture du risque de crédit
                  pour lancer l'étape.
                </p>
              )}
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <Button
                  size="sm"
                  variant={guarantee ? "outline" : "default"}
                  onClick={() => setDialog("guarantee")}
                >
                ...
                {guarantee?.document_url && (
                  <Button asChild size="sm" variant="outline">
                    <a href={guarantee.document_url} target="_blank" rel="noreferrer">
                      <FileText className="mr-1.5 h-4 w-4" aria-hidden /> Acte de garantie
                    </a>
                  </Button>
                )}
                {guarantee?.signed_document_url && (
                  <Button asChild size="sm" variant="outline">
                    <a href={guarantee.signed_document_url} target="_blank" rel="noreferrer">
                      <Download className="mr-1.5 h-4 w-4" aria-hidden /> Version signée
                    </a>
                  </Button>
                )}
              </div>
            ...
            <section className="rounded-xl border border-border p-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                {insurance && (
                  <Badge className={TONE[String(insurance.status)] ?? TONE.pending}>
                    {humanLabel(STATUS_LABEL, insurance.status, "En attente")}
                  </Badge>
                )}
              </div>
                  ...
                  <Field
                    ...
                    value={
                      insurance.scheduled_payment_date
                        ? new Date(insurance.scheduled_payment_date).toLocaleDateString("fr-FR")
                        : "—"
                    }
                  />
                  <Field
                    label="Numéro de police"
                    value={insurance.policy_number?.trim() || "En cours d'attribution"}
                  />
                  <Field
                    label="Garanties couvertes"
                    value={humanLabel(COVERAGE_LABEL, insurance.coverage)}
                  />
                  {insurance.document_hash && (
                    <Field
                      label="Empreinte SHA-256"
                      value={`${insurance.document_hash.slice(0, 16)}…`}
                    />
                  )}
                </div>
              ) : (
                <p className="mt-3 text-sm text-muted-foreground">
                  Étape distincte de la garantie : définissez la prime, les frais de mise en place
                  et l'échéance.
                </p>
              )}
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <Button
                  size="sm"
                  variant={insurance ? "outline" : "default"}
                  onClick={() => setDialog("insurance")}
                >
                  {insurance ? "Nouvelle assurance" : "Demander l'assurance"}
                </Button>
                {insurance &&
                  insurance.payment_status !== "paid" &&
                  Number(insurance.fee_amount ?? 0) > 0 && (
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => setDialog("insurancePayment")}
                    >
                ...
                {insurance?.document_url && (
                  <Button asChild size="sm" variant="outline">
                    <a href={insurance.document_url} target="_blank" rel="noreferrer">
                      <FileText className="mr-1.5 h-4 w-4" aria-hidden /> Notice d'assurance
                    </a>
                  </Button>
                )}
                {insurance?.signed_document_url && (
                  <Button asChild size="sm" variant="outline">
                    <a href={insurance.signed_document_url} target="_blank" rel="noreferrer">
                      <Download className="mr-1.5 h-4 w-4" aria-hidden /> Version signée
                    </a>
                  </Button>
                )}
        ...
        <p className="rounded-lg bg-muted/50 p-3 text-xs text-muted-foreground">
          Sécurité : un choix du client n'emporte aucun paiement. Le statut « payé » provient
          uniquement d'une validation administrative explicite ou d'un évènement de paiement du
          prestataire.
        </p>
      </CardContent>
      ...
      <Dialog open={dialog === "guarantee"} onOpenChange={(o) => (o ? null : close())}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Le montant saisi est enregistré dans le dossier, affiché au client et repris dans
              l'email.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <Input
                id="g-name"
                value={gForm.guarantor_name}
                onChange={(e) => setGForm({ ...gForm, guarantor_name: e.target.value })}
              />
            </div>
            <div>
              ...
              <Input
                id="g-desc"
                value={gForm.fee_description}
                onChange={(e) => setGForm({ ...gForm, fee_description: e.target.value })}
              />
            </div>
            <div>
      ...
      <Dialog open={dialog === "guaranteePayment"} onOpenChange={(o) => (o ? null : close())}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              À n'utiliser qu'après réception effective des fonds. Le dossier passe en « garantie
              validée ».
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div>
              ...
              <Input
                id="gp-ref"
                value={gPay.payment_reference}
                onChange={(e) => setGPay({ ...gPay, payment_reference: e.target.value })}
              />
            </div>
            <div>
              ...
              <Textarea
                ...
                rows={3}
                value={gPay.note}
                onChange={(e) => setGPay({ ...gPay, note: e.target.value })}
              />
            </div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={dialog === "insurance"} onOpenChange={(o) => (o ? null : close())}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Étape distincte de la garantie : prime mensuelle, frais de mise en place et échéance
              de paiement.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div>
              ...
              <Input
                id="i-prov"
                value={iForm.provider}
                onChange={(e) => setIForm({ ...iForm, provider: e.target.value })}
              />
            </div>
            <div>
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              ...
              <div>
                ...
                <Input
                  id="i-cur"
                  maxLength={3}
                  value={iForm.currency}
                  onChange={(e) => setIForm({ ...iForm, currency: e.target.value.toUpperCase() })}
                />
              </div>
            </div>
            <div>
              ...
              <Input
                id="i-desc"
                value={iForm.fee_description}
                onChange={(e) => setIForm({ ...iForm, fee_description: e.target.value })}
              />
            </div>
            <div>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-3">
            <div>
              ...
              <Input
                id="ip-ref"
                value={iPay.payment_reference}
                onChange={(e) => setIPay({ ...iPay, payment_reference: e.target.value })}
              />
            </div>
            <div>
              ...
              <Textarea
                ...
                rows={3}
                value={iPay.note}
                onChange={(e) => setIPay({ ...iPay, note: e.target.value })}
              />
            </div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={dialog === "insurancePolicy"} onOpenChange={(o) => (o ? null : close())}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Enregistrez la police et la date d'effet : le dossier passe en « assurance validée ».
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div>
              ...
              <Input
                id="ipol-num"
                value={iPolicy.policy_number}
                onChange={(e) => setIPolicy({ ...iPolicy, policy_number: e.target.value })}
              />
            </div>
            <div>
              ...
              <Input
                id="ipol-start"
                type="date"
                value={iPolicy.starts_on}
                onChange={(e) => setIPolicy({ ...iPolicy, starts_on: e.target.value })}
              />
            </div>
            <div>
              ...
              <Textarea
                id="ipol-note"
                rows={3}
                value={iPolicy.admin_notes}
                onChange={(e) => setIPolicy({ ...iPolicy, admin_notes: e.target.value })}
              />
            </div>
          </div>
          <DialogFooter>
