...
} from "@/components/finance/PortalUpload";
...
const STAGES: Array<{ key: string; statuses: ApplicationStatus[] }> = [
  ...
  {
    key: "contract",
    statuses: ["contract_sent", "signature_pending", "contract_signed"],
  },
  { key: "guarantee", statuses: ["guarantee_sent", "guarantee_signed"] },
  {
    key: "insurance",
    statuses: ["insurance_pending", "insurance_validated"],
  },
  {
    key: "disbursement",
    statuses: ["disbursement_preparing", "disbursed"],
  },
...
function SecurePortal() {
      ...
      const res = await chooseGuarantee({
        data: {
          ...
          scheduled_payment_date:
            choice === "pay_later" ? payDate : undefined,
        },
      });
    ...
    try {
      const res = await chooseInsurance({
        data: {
          ...
          choice: insChoice,
          scheduled_payment_date:
            insChoice === "pay_later" ? insDate : undefined,
        },
      });
  ...
  const num = (k: string) =>
    app[k] === null || app[k] === undefined ? null : Number(app[k]);

  const money = (v: number | null | undefined, cur = currency) =>
    v === null || v === undefined
      ? "—"
      : formatMoney(Number(v), cur, locale);
  ...
  const monthly =
    (num("monthly_payment") ?? 0) + (num("insurance_monthly") ?? 0);
  ...
  const stageState = STAGES.map((s) => {
    ...
    const isDone =
      !isCurrent && s.statuses.some((x) => reached.has(x));
  ...
  const lastDone = stageState.reduce(
    (acc, s, i) => (s.isDone || s.isCurrent ? i : acc),
    -1,
  );
  ...
  const progress =
    isTerminal(status) && !rejected
      ? 100
      : Math.round(((lastDone + 1) / STAGES.length) * 100);
  ...
  return (
    <div className="mx-auto w-full max-w-4xl overflow-x-hidden px-3 pb-20 pt-4 sm:px-6 sm:pb-24 sm:pt-8 lg:px-8">
      <div className="mb-4 flex justify-end">
        <LanguageSwitcher />
      </div>

      {/* ---------------------------- Header ---------------------------- */}
      <header className="overflow-hidden rounded-2xl border border-border bg-gradient-to-br from-primary/10 via-card to-card p-4 sm:p-7">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0 flex-1">
            ...
            <h1 className="mt-1 break-all font-mono text-xl font-bold tracking-tight sm:text-3xl">
          ...
          <div className="flex w-full min-w-0 flex-col items-start gap-2 sm:w-auto sm:shrink-0 sm:items-end">
            <div className="max-w-full">
              <StatusBadge status={status} />
            </div>

            <span className="inline-flex max-w-full items-center gap-1.5 rounded-full border border-border bg-background/70 px-3 py-1 text-[11px] font-medium text-muted-foreground">
              <ShieldCheck
        ...
        <div className="mt-4 flex flex-col gap-2 text-xs text-muted-foreground sm:flex-row sm:flex-wrap sm:items-center">
      ...
      {(() => {
        const pending = (s: unknown) =>
          typeof s === "string" &&
          !["paid", "not_required", "cancelled"].includes(s);

        const hasDue =
          pending(guarantee?.payment_status) ||
          pending(insurance?.payment_status);

        if (!hasDue) return null;

        return (
          <Card className="mt-5 flex flex-col gap-4 border-primary/40 bg-primary/5 p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5">
            ...
            <Button asChild className="w-full sm:w-auto sm:shrink-0">
              <Link
                to="/$lang/secure/application/$token/payment"
                params={{ lang, token }}
              >
      ...
      <Card className="mt-5 p-4 sm:p-5">
        <h2 className="text-sm font-semibold">
          {t("finance.portal.timeline")}
        </h2>

        <ol className="mt-4 space-y-3">
          {stageState.map((s, index) => (
            <li key={s.key} className="relative flex min-w-0 gap-3">
              ...
              <p
                className={cn(
                  "min-w-0 break-words pt-0.5 text-sm leading-relaxed",
        ...
        <Card className="p-4 sm:p-5 lg:col-span-3">
          ...
          <dl className="mt-4 grid gap-2 text-sm sm:grid-cols-2">
            ...
            <Row
              ...
              value={money(num("total_cost"))}
            />

            <Row
              label={t("finance.portal.interest")}
              value={money(num("total_interest"))}
            />

            <Row
              label={t("finance.portal.fees")}
              value={money(num("fees"))}
            />

            <Row
              label={t("finance.fields.iban")}
              value={maskIban(String(app.bank_iban ?? ""))}
            />

            <Row
              label={t("finance.fields.bankHolder")}
              value={String(app.bank_holder ?? "—")}
            />
          </dl>
        </Card>
          ...
          {history.length === 0 ? (
            ...
            <ol className="mt-4 space-y-4">
              {history.map((h, index) => {
                ...
                return (
                  <li
                    key={`${h.created_at}-${index}`}
                    className="relative flex min-w-0 gap-3 pb-1"
                  >
                    {!last && (
                      <span
      ...
      {infoRequests.length > 0 && (
        <Card
          id="info-requests"
          className="mt-5 scroll-mt-24 p-4 sm:p-5"
        >
          <h2 className="text-sm font-semibold">
            {t("finance.portal.requests.title")}
          </h2>
              ...
              >
                <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                  <p className="min-w-0 break-words text-sm font-medium">
                    {t(
                      `finance.portal.requests.kind${kindSuffix(r.kind)}`,
                      {
                        defaultValue: r.kind,
                      },
                    )}
                  ...
                  <span className="w-fit max-w-full shrink-0 rounded-full bg-muted px-2.5 py-1 text-[11px] text-muted-foreground">
                    {t(`finance.portal.requests.${r.status}`, {
                ...
                {r.status === "open" ? (
                  <div className="mt-3 space-y-2">
                    <textarea
                      className="min-h-20 w-full resize-y rounded-md border border-input bg-background p-3 text-sm"
                      value={replies[r.id] ?? ""}
                      onChange={(e) =>
                        setReplies((p) => ({
                          ...p,
                      ...
                      maxLength={2000}
                      placeholder={t(
                        "finance.portal.requests.placeholder",
                      )}
                    />
                  ...
                  r.response_text && (
                    <p className="mt-2 break-words rounded-md bg-muted p-3 text-sm leading-relaxed">
                      {r.response_text}
                    </p>
                  )
                )}
      ...
      >
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <h2 className="text-sm font-semibold">
            {t("finance.portal.documents")}
          </h2>

          <span className="w-fit max-w-full rounded-full bg-muted px-2.5 py-1 text-[11px] font-medium text-muted-foreground">
            ...
            {documents.map((d) => (
              ...
              >
                ...
                <Button
                  ...
                  onClick={() =>
                    void openDocument("document", d.id)
                  }
                >
        ...
        <div className="mt-4 rounded-xl border border-dashed border-border p-3 sm:p-4">
          <p className="text-sm font-medium">
            {t("finance.portal.upload.title")}
          </p>

          <p className="mt-1 break-words text-xs leading-relaxed text-muted-foreground">
            {t("finance.portal.upload.desc")}
          </p>
        ...
        <p className="mt-3 break-words text-[11px] leading-relaxed text-muted-foreground">
          {t("finance.portal.downloadNotice")}
        </p>
      </Card>

      {/* ------------------------------- KYC ----------------------------- */}
      <Card id="kyc" className="mt-5 scroll-mt-24 p-4 sm:p-5">
        <h2 className="text-sm font-semibold">
          {t("finance.portal.kycTitle")}
        </h2>

        {kycChecks.length === 0 ? (
          <p className="mt-4 text-sm text-muted-foreground">
            {t("finance.portal.kycNone")}
          </p>
        ) : (
          <ul className="mt-4 grid gap-2 sm:grid-cols-2">
            {kycChecks.map((c) => (
              <li
                key={c.id}
                className="flex min-w-0 flex-col gap-2 rounded-lg bg-muted/40 px-3 py-2 text-sm sm:flex-row sm:items-center sm:justify-between"
              >
      ...
      {/* ------------------------------ Offer ---------------------------- */}
      {offer && (
        <Card id="offer" className="mt-5 scroll-mt-24 p-4 sm:p-5">
          <h2 className="flex items-start gap-2 text-sm font-semibold">
            <BadgeEuro
              className="mt-0.5 h-4 w-4 shrink-0 text-primary"
              aria-hidden
          ...
          <dl className="mt-4 grid gap-2 text-sm sm:grid-cols-2">
            <Row
              label={t("finance.sim.amount")}
              value={money(offer.amount, offer.currency)}
            />
            <Row
              label={t("finance.sim.duration")}
              value={`${offer.duration_months} ${t(
                "finance.sim.months",
              )}`}
            />
            <Row
            ...
            />
            <Row
              label={t("finance.portal.validUntil")}
              value={date(offer.valid_until)}
            />
            <Row
              label={t("finance.portal.acceptedOn")}
              value={date(offer.accepted_at)}
            />
          </dl>
        </Card>
      )}
      ...
      {/* ----------------------------- Contract -------------------------- */}
      {contract && (
        <Card id="contract" className="mt-5 scroll-mt-24 p-4 sm:p-5">
          <h2 className="flex items-start gap-2 text-sm font-semibold">
            <FileSignature
              className="mt-0.5 h-4 w-4 shrink-0 text-primary"
              aria-hidden
          ...
          <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:flex-wrap">
            ...
            >
              <Link
                to="/$lang/secure/application/$token/contract"
                params={{ lang, token }}
              >
            ...
            {contract.has_signed_document && (
              <Button
                ...
                className="w-full gap-2 sm:w-auto"
                onClick={() =>
                  void openDocument("signed_contract")
                }
              >
                <Download className="h-4 w-4" aria-hidden />
      ...
      {/* ----------------------------- Guarantee ------------------------- */}
      {guarantee && (
        <Card id="guarantee" className="mt-5 scroll-mt-24 p-4 sm:p-5">
          <h2 className="flex items-start gap-2 text-sm font-semibold">
            <ShieldCheck
              className="mt-0.5 h-4 w-4 shrink-0 text-primary"
              aria-hidden
          ...
          <dl className="mt-4 grid gap-2 text-sm sm:grid-cols-2">
            <Row
              ...
              value={money(
                Number(guarantee.fee_amount ?? guarantee.amount),
                guarantee.currency,
              )}
            />
            ...
            <Row
              ...
              value={t(
                `finance.guarantee.payment.${
                  guarantee.payment_status ?? "unpaid"
                }`,
                {
                  defaultValue: String(
                    guarantee.payment_status ?? "—",
                  ),
                },
              )}
            />
            ...
            <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:flex-wrap">
              ...
              {guarantee.has_signed_document && (
                <Button
                  ...
                  className="w-full gap-2 sm:w-auto"
                  onClick={() =>
                    void openDocument("signed_guarantee")
                  }
                >
                  <Download className="h-4 w-4" aria-hidden />
                  {t("finance.portal.downloadSigned")}
          ...
          {guarantee.payment_status === "paid" ? (
            <p className="mt-4 break-words rounded-lg bg-success/10 p-3 text-sm leading-relaxed text-success">
            ...
            <div className="mt-4 space-y-3">
              <p className="break-words rounded-lg bg-muted/50 p-3 text-sm">
                {t(
                  `finance.guarantee.chosen.${guarantee.client_choice}`,
                  {
              ...
              {guarantee.client_choice !== "decline" &&
            ...
            <div className="mt-4 space-y-3">
              <p className="break-words text-sm font-medium">
                {t("finance.guarantee.question")}
              </p>

              <div className="space-y-2">
                {(
                  ["pay_now", "decline", "pay_later"] as GuaranteeChoice[]
                ).map((option) => (
                  <label
                    key={option}
                    className={cn(
                      "flex min-w-0 cursor-pointer items-start gap-3 rounded-xl border p-3 text-sm transition-colors",
                      choice === option
              ...
              {choice === "pay_later" && (
                <label className="block text-sm">
                  ...
                  <input
                    type="date"
                    className="h-10 w-full max-w-full rounded-md border border-input bg-background px-3 text-sm sm:w-auto"
                    value={payDate}
              ...
              <Button
                ...
                onClick={() =>
                  void submitGuaranteeChoice(guarantee.id)
                }
              >
      ...
      {/* ----------------------------- Insurance ------------------------- */}
      {insurance && (
        <Card id="insurance" className="mt-5 scroll-mt-24 p-4 sm:p-5">
          <h2 className="flex items-start gap-2 text-sm font-semibold">
            <Umbrella
              className="mt-0.5 h-4 w-4 shrink-0 text-primary"
              aria-hidden
          ...
          <dl className="mt-4 grid gap-2 text-sm sm:grid-cols-2">
            <Row
              label={t("finance.portal.provider")}
              value={insurance.provider ?? "—"}
            />

            <Row
              label={t("finance.portal.policy")}
            ...
            {insurance.due_date && (
              <Row
                label={t("finance.insurance.dueDate")}
                value={date(insurance.due_date)}
              />
            )}
            ...
            <Row
              label={t("finance.portal.signedOn")}
              value={date(insurance.validated_at)}
            />
          </dl>
            ...
            <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:flex-wrap">
              ...
              {insurance.has_signed_document && (
                <Button
                  ...
                  className="w-full gap-2 sm:w-auto"
                  onClick={() =>
                    void openDocument("signed_insurance")
                  }
                >
                  <Download className="h-4 w-4" aria-hidden />
                  {t("finance.portal.downloadSigned")}
          ...
          {insurance.payment_status === "paid" ? (
            ...
            <div className="mt-4 space-y-3">
              <p className="break-words rounded-lg bg-muted/50 p-3 text-sm">
                {t(
                  `finance.insurance.chosen.${insurance.client_choice}`,
                  {
              ...
              {insurance.client_choice !== "decline" &&
            ...
            <div className="mt-4 space-y-3">
              <p className="break-words text-sm font-medium">
                {t("finance.insurance.question")}
              </p>

              <div className="space-y-2">
                {(
                  ["pay_now", "decline", "pay_later"] as InsuranceChoice[]
                ).map((option) => (
                  <label
                    key={option}
                    className={cn(
                  ...
                  >
                    ...
                    <span className="min-w-0">
                      <span className="block break-words font-medium">
                        {t(
                          `finance.insurance.options.${option}`,
                        )}
                      </span>

                      <span className="mt-0.5 block break-words text-xs leading-relaxed text-muted-foreground">
                        {t(
                          `finance.insurance.optionsHint.${option}`,
                        )}
                      </span>
                    </span>
                  </label>
              ...
              <Button
                ...
                onClick={() =>
                  void submitInsuranceChoice(insurance.id)
                }
              >
      ...
      {disbursement && (
        ...
        >
          ...
          <dl className="mt-4 grid gap-2 text-sm sm:grid-cols-2">
            ...
            <Row
              label={t("finance.portal.beneficiary")}
              value={disbursement.beneficiary ?? "—"}
            />

            <Row
              label={t("finance.fields.iban")}
              value={disbursement.iban || "—"}
            />

            <Row
              label={t("finance.portal.sentOn")}
              value={date(disbursement.processed_at)}
            />
          </dl>
        </Card>
      )}
          ...
          <div className="mt-4 overflow-x-auto rounded-lg border border-border">
            <table className="w-full min-w-[520px] text-sm">
              <thead>
                <tr className="text-left text-xs uppercase tracking-wide text-muted-foreground">
                  <th className="py-2 pl-3 pr-3 font-medium">
                    {t("finance.portal.installment")}
                  </th>

                  <th className="py-2 pr-3 font-medium">
                    {t("finance.portal.dueDate")}
                  </th>
              ...
              <tbody className="divide-y divide-border">
                {repayments.map((r) => (
                  <tr key={r.id}>
                    <td className="py-2 pl-3 pr-3 tabular-nums">
                      {r.installment_no}
                    </td>
                    ...
                    <td className="py-2 pr-3 text-right">
                      ...
                      >
                        {r.paid
                          ? t("finance.portal.paid")
                          : t("finance.portal.unpaid")}
                      </span>
                    </td>
                  </tr>
                ))}
  ...
  return (
    <Card className="min-w-0 p-3.5 sm:p-4">
      <div className="flex min-w-0 items-start justify-between gap-2">
        <p className="min-w-0 break-words text-[10px] uppercase leading-tight tracking-wide text-muted-foreground sm:text-[11px]">
          {label}
        </p>
      ...
      <p className="mt-2 break-words font-serif text-lg font-semibold sm:text-xl">
        {value}
      </p>
    </Card>
  );
